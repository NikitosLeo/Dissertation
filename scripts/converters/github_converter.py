...
INPUT_DIR = Path("/opt/Digital_Footprint_System/result/github-monitor/new")
OUTPUT_DIR = Path("/opt/Digital_Footprint_System/result/github-monitor/jsonl")


def extract_find_domain(filename: str) -> str:
...
def load_json_flexible(src_path: Path):
    ...
    try:
        ...
        raise ValueError(f"{src_path}: JSON decode error: {e}")
...
def normalize_to_records(data, src_path: Path):
    ...
    if isinstance(data, dict):
        # Если это одиночный объект — превращаем в список из одного элемента
        return [data]
...
def convert_file(src_path: Path, dst_path: Path):
    ...
    find_domain = extract_find_domain(src_path.name)

    with dst_path.open("w", encoding="utf-8") as out:
        for item in records:
            record = {"find_domain": find_domain}
            record.update(item)
